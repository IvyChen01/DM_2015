...
class AdminController
{
	...
	{
		$this->checkLogin();
		include(Config::$templatesDir . 'admin/main.php');
	}
	...
	{
		$this->checkLogin();
		include(Config::$templatesDir . 'admin/change_password.php');
	}
	...
	private function db()
	{
		...
		foreach ($allTables as $tbName)
		{
			...
			$tableInfo['records'] = $install->getRecords($tbName, 0, 10000000);
			$_tableList[] = $tableInfo;
		}
		include(Config::$templatesDir . 'admin/db.php');
	}
	
	private function dbWish()
	{
		$this->checkLogin();
		$install = new Install();
		$allTables = array(Config::$tbUser, Config::$tbWish);
		$_tableList = array();
		foreach ($allTables as $tbName)
		{
			$tableInfo = array();
	...
	private function lucky()
	{
		...
		*/
		...
		if ($key == $srcKey && !empty($openId))
		{
			...
			include(Config::$templatesDir . 'hong_bao/main.php');
		}
		else
		{
			echo 'Request Error!';
...
class NewsController
{
	...
	private function listNews()
	{
		...
		include(Config::$templatesDir . 'news/list.php');
		
		/*
		if (Utils::checkMobile())
		{
			//手机版
			include(Config::$templatesDir . 'news/mobile_list.php');
		}
		else
		{
			//PC版
	...
	private function detailNews()
	{
		...
		{
			include(Config::$templatesDir . 'news/detail.php');
			
			/*
			if (Utils::checkMobile())
			{
				//手机版
				include(Config::$templatesDir . 'news/mobile_detail.php');
			}
			else
			{
				//PC版
...
class UserController
{
	...
	{
		$this->checkLogin();
		include(Config::$templatesDir . 'user/main.php');
	}
	...
	{
		$this->checkLogin();
		include(Config::$templatesDir . 'user/change_password.php');
	}
	...
	private function showLogin($isDataAction = false)
	{
		...
		{
			include(Config::$templatesDir . 'user/login.php');
		}
	}
}
	...
	private function lucky()
	{
		...
		*/
		...
		if ($key == $srcKey && !empty($openId))
		{
			...
			include(Config::$templatesDir . 'zhaun_pan/main.php');
		}
		else
		{
			echo 'Request Error!';
...
class Config
{
	...
	public static $key = '1faafa,.wishwall>{dfl2kj322';//密钥
...
class Install
{
	...
	private function createTable()
	{
		...
		Config::$db->query("drop table if exists $tbWish");
		Config::$db->query("create table $tbWish (
			id int not null auto_increment primary key,
			fbid varchar(50) not null,
	...
	public function upgrade()
	{
		...
		//Config::$db->query("delete from $tbWish");
	}
}
