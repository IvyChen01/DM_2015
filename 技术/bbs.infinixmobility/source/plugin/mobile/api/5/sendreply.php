...
class mobile_api {
	...
	function output() {
		global $_G;
		$phonename = $_GET['phone_type'];
		C::t('forum_post')->update('tid:' . $_G['tid'], $GLOBALS['pid'], array('phonetype' => $phonename));
		$variable = array();
		$result = mobile_core::variable($variable);
		if ($result['Message']['messageval'] == 'post_reply_succeed'){
		    $post_new = dheader('location: index.php?version=5&mobile=no&module=viewthread&tid='.$_G['tid'].'&viewpid='.$GLOBALS['pid'].'&from=&inajax=1&ajaxtarget=post_new');
		}
