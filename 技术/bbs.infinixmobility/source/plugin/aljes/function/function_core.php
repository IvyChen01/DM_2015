<?php
function generatethread($subject,$message,$ip,$uid,$username,$bkfid,$threadtypes){
	global $_G;
	...
	include_once libfile('function/post');
		...
		'$bkfid', '0', '0', '0','$threadtypes', '0', '$username', '$uid', '$subject', '$_G[timestamp]', '$_G[timestamp]', '$username', '0', '0', '$special', '0', '0', '0', '0', '32', '0')");
		$synctid = DB::insert_id();
		$tid=$synctid;
		$syncpid = insertpost(array('fid' => $bkfid,'tid' => $synctid,'first' => '1','author' => $username,'authorid' =>$uid,'subject' => $subject,'dateline' => $_G['timestamp'],'message' => $message,'useip' => $clientip,'invisible' => '0','anonymous' => '0','usesig' => '1','htmlon' => 0,'bbcodeoff' => 0,'smileyoff' => 0,'parseurloff' => '0','attachment' => '2',));
		$pid=$syncpid;
...
function generatepost($huitie,$uid,$tid,$fid,$Signiture,$mes){
	global $_G;
	$time=$_G['timestamp'];
	include_once libfile('function/forum');
	include_once libfile('function/post');
