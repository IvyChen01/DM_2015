...
function discuzcode_in_mobile($message, $smileyoff, $bbcodeoff, $htmlon = 0, $allowsmilies = 1, $allowbbcode = 1, $allowimgcode = 1, $allowhtml = 0, $jammer = 0, $parsetype = '0', $authorid = '0', $allowmediacode = '0', $pid = 0, $lazyload = 0, $pdateline = 0, $first = 0) {
    global $_G;
    ...
    if(!$smileyoff && $allowsmilies && (!defined('IN_MOBILE_API') || IN_MOBILE_API != 1)) {
        $message = parsesmiles($message);
    }
    ...
    if(!$bbcodeoff && $allowbbcode) {
        ...
        $message = str_replace(array(
            ...
            '</font>', '</font>', '</font>', '</font>', '</div>', '', '', '', '', '<hr class="l" />', '</p>', '', '',
        ...
        if($pid && !defined('IN_MOBILE') || defined('IN_MOBILE_API') && IN_MOBILE_API == 1) {
            $message = preg_replace("/\s?\[postbg\]\s*([^\[\<\r\n;'\"\?\(\)]+?)\s*\[\/postbg\]\s?/ies", "parsepostbg('\\1', '$pid')", $message);
        } else {
            $message = preg_replace("/\s?\[postbg\]\s*([^\[\<\r\n;'\"\?\(\)]+?)\s*\[\/postbg\]\s?/is", "", $message);
        }
            ...
            if(strpos($msglower, '[hide]') !== FALSE) {
                ...
                if($authorreplyexist) {
                    ...
                    if(defined('VIEW_THREAD_FROM_MOBLIE')){
                        $message = preg_replace("/\[hide\](.*?)\[\/hide\]/is", tpl_hide_reply_hidden_api_page(), $message);
                    }else{
                        $message = preg_replace("/\[hide\](.*?)\[\/hide\]/is", tpl_hide_reply_hidden(), $message);
                        $message = '<script type="text/javascript">replyreload += \',\' + '.$pid.';</script>'.$message;
                    }
    ...
    if(!$bbcodeoff) {
        ...
        /*if(defined('IN_MOBILE') && !defined('TPL_DEFAULT') && !defined('IN_MOBILE_API') || defined('IN_MOBILE_API') && IN_MOBILE_API == 1) {
         $allowimgcode = false;
         } */
        $attrsrc = !IS_ROBOT && $lazyload ? 'file' : 'src';
        if(strpos($msglower, '[/img]') !== FALSE) {
...
function parseimg($width, $height, $src, $lazyload, $pid, $extra = '') {
	...
	if($guestviewthumb) {
		...
		if(defined('IN_MOBILE')) {
		        $img = '<img'.($width > 0 ? ' width="'.$width.'"' : ' width="320px"').($height > 0 ? ' height="'.$height.'"' : '').' src="{url}" border="0" alt="" />';
		} else {
