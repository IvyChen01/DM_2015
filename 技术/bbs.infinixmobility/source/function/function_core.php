...
function get_avatar2($uid, $size = 'middle', $type = '') {
    $size = in_array($size, array('big', 'middle', 'small')) ? $size : 'middle';
    $uid = abs(intval($uid));
    $uid = sprintf("%09d", $uid);
    $dir1 = substr($uid, 0, 3);
...
function dgmdate($timestamp, $format = 'dt', $timeoffset = '9999', $uformat = '') {
	...
	if($format == 'u') {
		...
		if($timestamp >= $todaytimestamp) {
			...
			if(!defined('IN_MOBILE')) {
				$return = '<span title="'.$s.'" style="word-wrap:break-word;">'.$return.'</span>';
			}
		} else {
			$return = $s;
		}
...
/*vot debug*/	//return mb_strlen($str);
	$count = 0;
...
function getforumimg($aid, $nocache = 0, $w = 140, $h = 140, $type = '') {  
	global $_G;
	$key = dsign($aid.'|'.$w.'|'.$h);
	if(defined('IN_MOBILE') && IN_MOBILE && !defined('IN_MOBILE_API')){
            return $_G['siteurl'].'forum.php?mod=image&aid='.$aid.'&size='.$w.'x'.$h.'&key='.rawurlencode($key).($nocache ? '&nocache=yes' : '').($type ? '&type='.$type : '');		
	}else{
