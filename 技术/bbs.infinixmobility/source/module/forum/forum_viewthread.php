...
$_GET['from'] = in_array($_GET['from'], array('preview', 'portal', 'album','api','activitypreview','ajax')) ? $_GET['from'] : '';
defined('IN_MOBILE') && $_GET['from'] == 'api' && define('VIEW_THREAD_FROM_MOBLIE', 1);
...
if($postusers) {
	...
	foreach($postlist as $pid => $post) {
		...
		//var_dump($postlist[$pid]['api_groupicon']);exit;
	}
}
...
if(empty($_GET['viewpid'])) {
	...
	if($_GET['from'] == 'portal') {
		...
		$_G['widthauto'] = 0;
		$sufix = '_preview';
	} elseif($_GET['from'] == 'activitypreview') {
		    $_G['disabledwidthauto'] = 1;
		    $_G['widthauto'] = 0;
		    $sufix = '_activitypreview';
	...
	if(defined('VIEW_THREAD_FROM_MOBLIE')){
	    $sufix = '_api';
	}elseif($_GET['from']=='ajax'){
	    $sufix = '_ajax';
	}
...
function viewthread_procpost($post, $lastvisit, $ordertype, $maxposition = 0) {
	...
	* ==============================================
	* @Description : api页面显示作者等级图标等信息
	* @date: 2015年8月21日 上午11:28:03
	* @author: yanhui.chen
	* @version:
	*/
	$group_title = strip_tags($_G['cache']['usergroups'][$post['groupid']]['grouptitle']);
	if(substr($group_title,-3,2)=='LV'){
	    $grouptitle =substr($group_title,-3);
	    if (strpos($group_title, 'Jade Star')!==false){
	        $iconsrc = $_G['siteurl'].'static/image/level_api/jade.png';
	        $title_color = "#009b7a";
	    }elseif (strpos($group_title, 'Sapphire Star')!==false){
	        $iconsrc = $_G['siteurl'].'static/image/level_api/sapphire.png';
	        $title_color = "#2b749f";
	    }elseif (strpos($group_title, 'Golden Star')!==false){
	        $iconsrc = $_G['siteurl'].'static/image/level_api/golden.png';
	        $title_color = "#e5a134";
	    }elseif (strpos($group_title, 'Jade Diamond')!==false){
	        $iconsrc = $_G['siteurl'].'static/image/level_api/jade_diamond.png';
	        $title_color = "#009b7a";
	    }elseif (strpos($group_title, 'Sapphire Diamond')!==false){
	        $iconsrc = $_G['siteurl'].'static/image/level_api/sapphire_diamond.png';
	        $title_color = "#2b749f";
	    }elseif (strpos($group_title, 'Golden Diamond')!==false){
	        $iconsrc = $_G['siteurl'].'static/image/level_api/golden_diamond.png';
	        $title_color = "#e5a134";
	    }elseif (strpos($group_title, 'Crown')!==false){
	        $iconsrc = $_G['siteurl'].'static/image/level_api/crown.png';
	        $title_color = "#e5a134";
	    }
	    $post['api_grouptitle'] = $grouptitle;
	    $post['api_groupicon'] = $iconsrc;
	    $post['api_titlecolor'] = $title_color;
	}
	if ($post['gender'] == 1){
	    $gendersrc = $_G['siteurl'].'static/image/level_api/male.png';
	}elseif ($post['gender'] == 2){
	    $gendersrc = $_G['siteurl'].'static/image/level_api/female.png';
	}
	$post['api_gendericon'] = $gendersrc;
	//var_dump($post['api_groupicon']);exit;
	return $post;
}
