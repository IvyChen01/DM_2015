...
if(!empty($orderid)) {
	...
	if(submitcheck('tradesubmit')) {
		...
		if($tradelog['status'] == 0) {
			...
			if($update) {
				...
				$update['price'] = $price + ($tradelog['transport'] == 2 ? $tradelog['transportfee'] : 0) - $tradelog['credit']/100;
    ...
    $trade['offset'] = floor(getuserprofile('extcredits'.$_G['setting']['creditstransextra'][5])/100);
    $trade['xgold'] = getuserprofile('extcredits'.$_G['setting']['creditstransextra'][5]);
	$limit = 6;
	...
	if($_GET['action'] != 'trade' && !submitcheck('tradesubmit')) {
		...
		if($_G['setting']['creditstransextra'][5] != -1 && $trade['credit']) {
		    ...
		    $credit = $_GET['xgold']>=100 ? $_GET['xgold'] : 0;  //陈焰辉 2015.12.22 修改积分兑换
		}
        
		$price = $pay['price'] * $pay['number'] - floor($_GET['xgold']/100); //陈焰辉 2015.12.22 修改积分兑换
		$buyercredits = 0;
		$pay['commision'] = 0;
