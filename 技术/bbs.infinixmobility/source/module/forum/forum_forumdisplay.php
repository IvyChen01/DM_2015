...
$nation_arr = C::t('forum_threadclass')->fetch_nations_by_fid(41);
...
if($_G['forum']['type'] == 'forum') {
	...
	if(empty($_GET['archiveid'])) {
		...
		if ($_G['forum']['fid'] == '41'){
			$navigation = ' <a href="forum.php?mod=forumdisplay&fid='.$_G['forum']['fid'].'">'.$_G['forum']['name'].'</a>';
		}
	} else {
...
if($filter && $filter != 'hot') {
	if($query_string = $_SERVER['QUERY_STRING']) {
		...
		/* 根据客户端的IP定为到国家版块 2015-8-5 陈焰辉 */
		/*require_once libfile('function/misc');
		//$location = convertip($_G['clientip']);
		$location = convertip('41.131.123.54');
		if ($_G['fid'] == 41){
		    $geturl['filter'] = 'typeid';
		    switch ($location){
		        case 'Egypt': $geturl['typeid'] =!empty($_GET['typeid'])?$_GET['typeid']:9;
		        break;
		        case "Cote d'Ivoire": $geturl['typeid'] =!empty($_GET['typeid'])?$_GET['typeid']:20;
		        break;
		        case "France": $geturl['typeid'] = !empty($_GET['typeid'])?$_GET['typeid']:10;
		        break;
		        case "Ghana": $geturl['typeid'] = !empty($_GET['typeid'])?$_GET['typeid']:11;
		        break;
		        case "Kenya": $geturl['typeid'] = !empty($_GET['typeid'])?$_GET['typeid']:12;
		        break;
		        case "Morocco": $geturl['typeid'] =!empty($_GET['typeid'])?$_GET['typeid']: 13;
		        break;
		        case "Nigeria": $geturl['typeid'] = !empty($_GET['typeid'])?$_GET['typeid']:14;
		        break;
		        case "Pakistan": $geturl['typeid'] =!empty($_GET['typeid'])?$_GET['typeid']: 15;
		        break;
		        case "Saudi Arabia": $geturl['typeid'] =!empty($_GET['typeid'])?$_GET['typeid']: 16;
		        break;
		        case "United Arab Emirates": $geturl['typeid'] =!empty($_GET['typeid'])?$_GET['typeid']: 17;
		        break;
		        default:break;
		    }
...
foreach($threadlist as $thread) {
	 ...
	 * ==============================================
	 * @Description :列表中不显示交易帖子
	 * @date: 2015年7月7日 下午2:31:12
	 * @author: yanhui.chen
	 * @version:
	 */
	if ($thread['special'] == 2){
