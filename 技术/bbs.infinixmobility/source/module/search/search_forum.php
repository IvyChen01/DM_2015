...
if((!defined('IN_MOBILE_API') || !IN_MOBILE_API) &&!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		if(defined('IN_MOBILE_API') && IN_MOBILE_API){
			$page = max(1, intval($_GET['page']));
			dheader("location: ".$_G['siteurl'] . "api/mobile/index.php?mobile=no&version=".getgpc('version')."&module=searchthread&searchid=$searchid&orderby=$orderby&ascdesc=$ascdesc&kw=".urlencode($keyword)."&page=".$page);
		}else{
