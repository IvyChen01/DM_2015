...
$op = $_GET['op'];
if(!in_array($op, array('basic', 'trade', 'team', 'trend', 'modworks', 'memberlist','memberlist_reg', 'forumstat', 'trend'))) {
	$op = 'basic';
}
...
}elseif ($op == 'memberlist_reg'){
    $statvars = getstatvars('memberlist2');
    extract($statvars);
    if (!empty($_GET['nation'])){
        $nation = $_GET['nation'];
        $userlist = $statvars['userlist'][$nation];
        foreach ($userlist as $key=>$value){
            $credits[] = $value['extcredits2'];
        }
        array_multisort($credits, SORT_DESC, $userlist);
        $num = count($userlist);
        $page = $_GET['page'];
        if(empty($page)) {
            $page = 1;
        ...
        $userlist = array_slice($userlist, $number-$pagenumber,$pagenumber);
        $multipage = multi($num, $pagenumber, $page, 'misc.php?mod=stat&op=memberlist_reg&nation='.$nation.'&primarybegin='.$statvars[begin].'&primaryend='.$statvars[end], $_G['setting']['membermaxpages']);
    }
    include template('forum/stat_memberlist_c');
} elseif($op == 'forumstat') {
...
function getstatvars_memberlist2() {
    
    global $_G;
    $statvars = array();
    
    if (!empty($_GET['primarybegin'])){
        $begin = $_GET['primarybegin'];
    }
    if (!empty($_GET['primaryend'])){
        $end = $_GET['primaryend'];
    }
	...
	$filterarr['begin'] = strtotime($begin);
	$filterarr['end'] = strtotime($end);
    
    
    $memberlist = C::t('common_member')->fetch_all_stat_memberlist_reg($filterarr, $_GET['order'], $_GET['asc'] ? 'ASC' : 'DESC', $start_limit, $_G['setting']['memberperpage']);
    
    foreach ($memberlist as $k=>$v){
        if ($v['nation'] == 'Nigeria'){
            $statvars['country']['Nigeria'] += 1;
            $statvars['userlist']['Nigeria'][] = $v;
        }elseif ($v['nation'] == 'Egypt'){
            $statvars['country']['Egypt'] += 1;
            $statvars['userlist']['Egypt'][] = $v;
        }elseif ($v['nation'] == 'Kenya'){
            $statvars['country']['Kenya'] += 1;
            $statvars['userlist']['Kenya'][] = $v;
        }elseif ($v['nation'] == 'United Arab Emirates'){
            $statvars['country']['United Arab Emirates'] += 1;
            $statvars['userlist']['United Arab Emirates'][] = $v;
        }elseif ($v['nation'] == 'Ghana'){
            $statvars['country']['Ghana'] += 1;
            $statvars['userlist']['Ghana'][] = $v;
        }elseif ($v['nation'] == "Cote D'Ivoire"){
            $statvars['country']["Cote D'Ivoire"] += 1;
            $statvars['userlist']["Cote D'Ivoire"][] = $v;
        }elseif ($v['nation'] == "Pakistan"){
            $statvars['country']["Pakistan"] += 1;
            $statvars['userlist']['Pakistan'][] = $v;
        }elseif ($v['nation'] == "Saudi Arabia"){
            $statvars['country']["Saudi Arabia"] += 1;
            $statvars['userlist']['Saudi Arabia'][] = $v;
        }elseif ($v['nation'] == "Morocco"){
            $statvars['country']["Morocco"] += 1;
            $statvars['userlist']['Morocco'][] = $v;
        }elseif ($v['nation'] == "France"){
            $statvars['country']["France"] += 1;
            $statvars['userlist']['France'][] = $v;
        }elseif ($v['nation'] == "Indonesia"){
            $statvars['country']["Indonesia"] += 1;
            $statvars['userlist']['Indonesia'][] = $v;
        }elseif ($v['nation'] == "Thailand"){
            $statvars['country']["Thailand"] += 1;
            $statvars['userlist']['Thailand'][] = $v;
        }else {
            $statvars['country']["other"] += 1;
            $statvars['userlist']['other'][] = $v;
        }
    }
    if (!empty($_GET['nation'])){
        $nation = $_GET['nation'];
    }
    $statvars['nation'] =$nation;
    $statvars['begin'] = $begin;
    $statvars['end'] = $end;
    $statvars['total'] = count($memberlist);
    return $statvars;
}
/**
