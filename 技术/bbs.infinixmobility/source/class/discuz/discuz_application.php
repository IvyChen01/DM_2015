...
/*vot*/		}
	...
	private function _init_user() {
		if($this->init_user) {
			...
			* ==============================================
			* @Description :手机API登录 需要客户端传回 auth 与 saltkey 值
			* @date: 2015年6月24日 下午2:48:50
			* @author: yanhui.chen
			* @version:
			*/
			if (defined('IN_MOBILE_API') && IN_MOBILE_API == 1 || $_GET['from'] == 'api' && checkmobile()) {
			    if ($_GET['saltkey'] && $_GET['auth']) {
			        $this->var['cookie']['saltkey'] = $_GET['saltkey'] == 'null' ? '' : $_GET['saltkey'];
			        $this->var['authkey'] = md5($this->var['config']['security']['authkey'] . $this->var['cookie']['saltkey']);
			        $auth = $this->var['cookie']['auth'] = str_replace(' ', '+', $_GET['auth']);
			        $auth = daddslashes(explode("\t", authcode($auth, 'DECODE')));
			    ...
			    if (strtolower($this->var['cookie']['saltkey']) == 'null') {
			        $this->var['cookie']['saltkey'] = '';
			    }
			}
	...
	private function _init_mobile() {
		...
		if(!$this->var['mobile'] && !$unallowmobile) {
			if($mobileflag) {
			    showmessage('not mobile, will direct to index','forum.php?mod=index');
