...
class discuz_database {
	...
	public static function fetch($resourceid, $type = 'MYSQL_ASSOC') {
	    ...
	    $result = self::$db->fetch_array($resourceid, $type);
	    
	    if ($result["username"]){
	        $result["realUsername"] = $result["username"];
	        if (!$fromAdmin){
		...
		while ($row = self::$db->fetch_array($query)) {
			if ($keyfield && isset($row[$keyfield])) {
			    ...
			    if ($row["username"]){
			     ...
			     if ($row["author"]){
			         $row["realAuthor"] = $row["author"];
			         if (!$fromAdmin){
			             $memResult = C::t('common_member')->fetch($row['authorid']);
			             if ($memResult["nickname"] != null){
			                 $row["author"] = $memResult["nickname"];
			             }
			          }
	 			}
			    if ($row["lastposter"]){
			         $row["realLastposter"] = $row["lastposter"];
			         if (!$fromAdmin){
			               $memResult = DB::fetch_first("SELECT nickname FROM ". DB::table('common_member') . " WHERE username='$row[lastposter]'");
			               if ($memResult["nickname"] != null){
			                        $row["lastposter"] = $memResult["nickname"];
			               }
			         }
			    }
			/* Modifier Modify */
	...
	public static function result($resourceid, $row = 0) {
	    ...
	    /* Modifier Modify */
	    Global $fromAdmin;
	    $result = self::$db->result($resourceid, $row);
	    if ($result["username"]){
	        $result["realUsername"] = $result["username"];
	        if (!$fromAdmin){
	            if ($result["nickname"] != null){
