...
class forum_upload {
	...
	function forum_upload($getaid = 0) {
		...
		$tmp_name_arr = split(".", $_FILES['Filedata']['name']);
		if (count($tmp_name_arr) < 2) {
			$_FILES['Filedata']['name'] .= ".jpg";
		}
	...
	function uploadmsg($statusid) {
		...
		if(defined('IN_MOBILE_API') && IN_MOBILE_API){
			if($this->getaid) {
				$_G['upload_result_id'] = $this->getaid = $statusid ? -$statusid : $this->aid;
			}
			if($this->simple == 1) {
				$_G['upload_result_id'] = 'DISCUZUPLOAD|'.$statusid.'|'.$this->aid.'|'.$this->attach['isimage'].'|'.$this->error_sizelimit;
			} elseif($this->simple == 2) {
				$_G['upload_result_id'] = 'DISCUZUPLOAD|'.($_GET['type'] == 'image' ? '1' : '0').'|'.$statusid.'|'.$this->aid.'|'.$this->attach['isimage'].'|'.($this->attach['isimage'] ? $this->attach['attachment'] : '').'|'.$this->attach['name'].'|'.$this->error_sizelimit;
			} else {
				$_G['upload_result_id'] = $statusid ? -$statusid : $this->aid;
			}
			return ;
		}else{
			if($this->getaid) {
