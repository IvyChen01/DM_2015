...
class table_forum_forumfield extends discuz_table
{
	...
	* ==============================================
	* @Description :获取版块的帖子分类
	* @date: 2015年8月6日 上午10:53:13
	* @author: yanhui.chen
	* @version:
	*/
	public function fetch_info_threadtypes_by_fid($fid){
	    $data=DB::fetch_first("SELECT threadtypes FROM %t WHERE fid = %n", array($this->_table, $fid));
	    return unserialize($data['threadtypes']);
	}
}
