...
class table_common_usergroup extends discuz_table
{
	...
	/**
	     ...
	     * @return return_type
	     * @author : yanhui.chen
	     * @date: 2015年6月17日 上午10:23:47
	     */
	public function userslevel() {
	    return DB::fetch_all("SELECT groupid,grouptitle,creditshigher,creditslower FROM %t WHERE type='member'",array($this->_table));
	}    
	public function fetch_by_groupid($gid) {
	    if(!$gid) {
	        return null;
	    }
	    return DB::fetch_all('SELECT * FROM %t WHERE groupid=%n', array($this->_table, $gid), $this->_pk);
	}
}
