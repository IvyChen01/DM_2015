...
class table_forum_ratelog extends discuz_table
{
	...
	/**
	     ...
	     * @return return_type
	     * @author : yanhui.chen
	     * @date: 2015年8月17日 下午2:50:49
	     */
	    
    public function fetch_ratenum_by_pid($pid,$score){
        if ($score>=0){
            $rate_num = DB::fetch_first('SELECT COUNT(*) FROM %t WHERE pid=%d AND score>=%d', array($this->_table, $pid,$score));
        }else {
            $rate_num = DB::fetch_first('SELECT COUNT(*) FROM %t WHERE pid=%d AND score<=%d', array($this->_table, $pid,$score));
        }
        return $rate_num;
