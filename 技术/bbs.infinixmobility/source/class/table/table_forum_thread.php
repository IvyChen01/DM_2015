...
class table_forum_thread extends discuz_table
{
	...
	public function search_condition($conditions, $prefix = false) {
		...
		if ($conditions['inforum'] == '41' && ( $conditions['intype'] == '' || !isset($conditions['intype'] ) ) )
			$wheresql .= " AND `typeid`!='34'";
	 ...
	 * @author Cogie
	 */
	public function fetch_feilds_search($conditions,$fields="*", $tableid = 0, $start = 0, $limit = 0, $order = '', $sort = 'DESC', $forceindex='') {
	    $ordersql = '';
	    ...
	    $data = DB::fetch_all("SELECT $fields FROM ".DB::table($this->get_table_name($tableid))." $forceindex".$this->search_condition($conditions)." $ordersql ".DB::limit($start, $limit));
