...
class table_forum_post extends discuz_table
{
	 ...
	 */
	public function fetch_feilds_by_tid($tableid, $tids,$feilds='*', $outmsg = true, $order = '', $start = 0, $limit = 0, $first = null, $invisible = null, $authorid = null, $fid = null) {
	    $postlist = $sql = array();
	    ...
	    $query = DB::query("SELECT $feilds FROM %t WHERE ".DB::field('tid', $tids).' %i '.($order ? 'ORDER BY dateline '.$order : '').' '.DB::limit($start, $limit),
	        array(self::get_tablename($tableid), ($sql ? 'AND '.implode(' AND ', $sql) : '')));
	    while($post = DB::fetch($query)) {
	        if(!$outmsg) {
	            unset($post['message']);
	...
	/**
	     * @Description: 通过tid，pid获取帖子内容
	     * @param variable
	     * @return return_type
	     * @author : yanhui.chen
	     * @date: 2015年7月15日 下午3:41:55
	     */
	public function fetch_message_by_tid_pid($tid, $pid) {
	    return DB::result_first('SELECT message FROM %t WHERE tid=%d AND pid=%d LIMIT 1', array(self::get_tablename('tid:'.$tid), $tid, $pid));
	}
	/**
	* Copyright @ 2013 Infinix. All rights reserved.
	* ==============================================
	* @Description :根据tid,pid获取内容
	* @date: 2015年7月31日 上午10:47:11
	* @author: yanhui.chen
	* @version:
	*/
	public function fetch_all_by_tid_pid($tid, $pid) {
	    return DB::fetch_first('SELECT * FROM %t WHERE tid=%d AND pid=%d LIMIT 1', array(self::get_tablename('tid:'.$tid), $tid, $pid));
	}
}
?>