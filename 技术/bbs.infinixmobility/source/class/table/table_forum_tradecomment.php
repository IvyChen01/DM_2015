...
class table_forum_tradecomment extends discuz_table
{
	...
	/* 根据pid获取信息 */
	function fetch_all_list_by_pid($pid) {
	    
	    $sql .= ' tc.type=0 AND tc.pid='.$pid ;
	
	    return DB::fetch_all("SELECT tc.*, tl.subject, tl.price, tl.credit FROM %t tc LEFT JOIN %t tl USING(orderid) WHERE %i ORDER BY tc.dateline DESC ",
	        array($this->_table,'forum_tradelog', $sql));
	}
