...
class table_common_member extends discuz_table_archive
{
	...
	public function fetch_by_username($username, $fetch_archive = 0) {
		$user = array();
		if($username) {
			$user = DB::fetch_first('SELECT * FROM %t WHERE username=%s or nickname=%s', array($this->_table, $username,$username));
	...
	* ==============================================
	* @Description :查询时间段内注册用户
	* @date: 2015年7月30日 下午2:05:46
	* @author: yanhui.chen
	* @version:
	*/
	public function fetch_all_stat_memberlist_reg($conditions, $orderby = '', $sort = '', $start = 0, $limit =  0) {
	    
	    $orderby = in_array($orderby, array('uid','credits','regdate', 'gender','username','posts','lastvisit'), true) ? $orderby : 'uid';
	    $sql = '';
	    
	    /* if ($conditions['nation']){
	        $wherearr[] = DB::field('nationality', $conditions['nation']);
	    } */
	    //$sql = !empty($username) ? " WHERE username LIKE '".addslashes(stripsearchkey($username))."%'" : '';
	    $wherearr['begin'] = DB::field('regdate', $conditions['begin'],'>=');
	    $wherearr['end'] = DB::field('regdate', $conditions['end'],'<=');
	    $wheresql2 = !empty($wherearr) && is_array($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
	    require_once libfile('function/misc');
	    $memberlist = array();
	    $query = DB::query("SELECT m.uid, m.username, mp.gender, m.email, m.regdate, ms.lastvisit, mc.posts,ms.regip, m.credits,mc.extcredits2
			FROM ".DB::table($this->_table)." m
			LEFT JOIN ".DB::table('common_member_profile')." mp ON mp.uid=m.uid
			LEFT JOIN ".DB::table('common_member_status')." ms ON ms.uid=m.uid
			LEFT JOIN ".DB::table('common_member_count')." mc ON mc.uid=m.uid
	        $wheresql2 ORDER BY ".DB::order($orderby, $sort));

	    while($member = DB::fetch($query)) {
	        $member['usernameenc'] = rawurlencode($member['username']);
	        $member['regdate'] = dgmdate($member['regdate']);
	        $member['lastvisit'] = dgmdate($member['lastvisit']);
	        $member['nation'] = convertip($member['regip']);
	        $memberlist[$member['uid']] = $member;
	    }
	    return $memberlist;
	}
    public function count_by_fields($fields){
        if($fields['username']) {
            $wherearr[] = DB::field('username', '%'.addslashes(stripsearchkey($fields['username'])).'%','like');
        }
        if ($fields['nation']){
            $wherearr[] = DB::field('nationality', $fields['nation']);
	...
	public function insert($uid, $username, $password,  $email, $ip, $groupid, $extdata, $adminid = 0, $nick=false) {
		if(($uid = dintval($uid))) {
