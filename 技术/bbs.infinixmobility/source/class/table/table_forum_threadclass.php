...
class table_forum_threadclass extends discuz_table
{
    ...
    /**
         ...
         * @return return_type
         * @author : yanhui.chen
         * @date: 2015年5月8日 下午3:31:49
         */
	public function fetch_nations_by_fid($fid) {
	    $nations = DB::fetch_all('SELECT typeid,name FROM %t WHERE fid=%d and moderators=0 ORDER BY displayorder', array($this->_table, $fid), $this->_pk);
	    foreach ($nations as $nation){
	        if ($nation['name'] != 'Other Country' && $nation['name'] != 'Service'){
	            $nation_arr[$nation['typeid']] = $nation;
	        }
	    }
	    return $nation_arr;
	}
	...
	* ==============================================
	* @Description :
	* @date: 2015年6月29日 下午12:03:04
	* @author: yanhui.chen
	* @version:
	*/ 
	public function fetch_typeid_by_fid($fid) {
	    $nations = DB::fetch_all('SELECT typeid,name FROM %t WHERE fid=%d ORDER BY displayorder', array($this->_table, $fid));
	    return $nations;
	}    
}
