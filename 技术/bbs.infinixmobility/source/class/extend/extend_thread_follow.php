...
class extend_thread_follow extends extend_thread_base {
	...
	public function after_newthread() {
		...
		if($this->param['displayorder'] >= 0 && helper_access::check_module('follow') && !$this->param['isanonymous']) {
			...
			$follow_after = followcode($this->param['message'], $tid, $pid, 1000);
			
			$lcount = mb_substr_count(substr($follow_after, 0, 1001), "<img");
			$rcount = mb_substr_count(substr($follow_after, 0, 1001), "/>");
			
			if ($lcount != $rcount){
				$follow_after_right = substr($follow_after, strpos($follow_after, "<a"));
				$follow_after_left = substr($follow_after, 0, strrpos($follow_after, "<img"));			
				$follow_after = $follow_after_left . $follow_after_right;	
			}
			
			$feedcontent = array(
				'tid' => $tid,
				'content' => $follow_after,
			);
