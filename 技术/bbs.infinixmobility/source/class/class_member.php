...
class logging_ctl
{
    ...
    function logging_more($questionexist, $secchecklogin2 = 0)
    {
        ...
        showmessage('location_login', '', array(
            'type' => 1
        ), array(
            'extrajs' => $js
        ));
    ...
    function on_loginOutsideLogin()
    {
        ...
        if (empty($_GET['username'])) {
            ...
            /*用Facebook账号的Email代替以前的  */
            $email = $_GET['email'];
            $nickname = addslashes(trim($_GET['nick']));
            if ($email != $result['ucresult']['email']){
                C::t('common_member')->update($uid, array('email' =>$email));
                $result['ucresult']['email'] = $email;
            }
            if ($email != $result['member']['email']){
            ...
            if ($result['status'] == - 1) {
                if (! $this->setting['fastactivation']) {
                    ...
                    C::t('common_member')->insert($uid, $result['ucresult']['username'], md5(random(10)), $result['ucresult']['email'], $_G['clientip'], $groupid, $init_arr,0,$nickname);
...
class register_ctl
{
    ...
    function register_ctl()
    {
        ...
        if ($_G['setting']['bbclosed']) {
            if (($_GET['action'] != 'activation' && ! $_GET['activationauth']) || ! $_G['setting']['closedallowactivation']) {
                showmessage('register_disable', NULL, array(), array(
                    'login' => 1
                ));
            }
        }
    ...
    function on_registerbyphone()
    {
        global $_G;
        $_GET['username'] = trim($_GET['phone']);
        ...
        $_GET['password2'] = $_GET['' . $this->setting['reginput']['password2']];
        // $_GET['email'] = $_GET['username']."@afmobi.group";
        // print_r($_G);exit;
        // $p = $_GET['email'];
        ...
        if ($_G['uid']) {
            ...
            showmessage('login_succeed', $url_forward ? $url_forward : './', array(
                'username' => $_G['member']['username'],
                'usergroup' => $_G['group']['grouptitle'],
                'uid' => $_G['uid']
            ), array(
            ...
            if ($_GET['action'] == 'activation' || $_GET['activationauth']) {
                ...
                showmessage(! $this->setting['regclosemessage'] ? 'register_disable' : str_replace(array(
                    "\r",
                    "\n"
        ...
        if (empty($_GET['username'])) {
            ...
            $bbrulestxt = nl2br("\n$bbrulestxt\n\n");
            $dreferer = dreferer();
            $username = $_G['setting']['mobile_phone'];
            if (uc_get_user(addslashes($username))) {
                // $this->template = 'member/login';
                showmessage('register_activation_message', 'member.php?mod=logging&action=login', array(
                    'username' => $username
                ));
            }
            ...
            if (! $activation) {
                ...
                if ($this->setting['pwlength']) {
                    if (strlen($_GET['password']) < $this->setting['pwlength']) {
                        showmessage('profile_password_tooshort', '', array(
                            'pwlength' => $this->setting['pwlength']
                        ));
                    }
                }
            ...
            if ($this->setting['regfloodctrl']) {
                ...
                if ($regip && $_G['clientip'] != '54.77.140.112') {    /* 限制同一IP注册过多用户除54.77.140.112外  */
                    if ($regip['count'] >= $this->setting['regfloodctrl']) {
                        showmessage('register_flood_ctrl', NULL, array(
                            'regfloodctrl' => $this->setting['regfloodctrl']
                        ));
                    } else {
                        $setregip = 1;
                    }
            ...
            foreach ($_G['cache']['fields_register'] as $field) {
                ...
                if (! profile_check($field_key, $field_val)) {
                    $showid = ! in_array($field['fieldid'], array(
                        'birthyear',
                        'birthmonth'
                    )) ? $field['fieldid'] : 'birthday';
                    showmessage($field['title'] . lang('message', 'profile_illegal'), '', array(), array(
                        'showid' => 'chk_' . $showid,
                        'extrajs' => $field['title'] . lang('message', 'profile_illegal') . ($field['formtype'] == 'text' ? '<script type="text/javascript">' . '$(\'registerform\').' . $field['fieldid'] . '.className = \'px er\';' . '$(\'registerform\').' . $field['fieldid'] . '.onblur = function () { if(this.value != \'\') {this.className = \'px\';$(\'chk_' . $showid . '\').innerHTML = \'\';}}' . '</script>' : '')
                    ));
                }
                if ($field['needverify']) {
            ...
            if (getuserbyuid($uid, 1)) {
                ...
                showmessage('profile_uid_duplicate', '', array(
                    'uid' => $uid
                ));
            }
            ...
            $init_arr = array(
                'credits' => explode(',', $this->setting['initcredits']),
                'profile' => $profile,
                'emailstatus' => $emailstatus
            );
            
            // C::t('common_member')->insert($uid, $username, $password, $email, $_G['clientip'], $groupinfo['groupid'], $init_arr);
            C::t('common_member')->insert($uid, $username, $password, $email, $_G['clientip'], $groupinfo['groupid'], $init_arr, 0, $nickname);
            if ($emailstatus) {
            ...
            if ($verifyarr) {
                ...
                C::t('common_member_verify')->insert(array(
                    'uid' => $uid
                ));
            }
            ...
            if ($invite['uid']) {
                if ($this->setting['inviteconfig']['inviteaddcredit']) {
                    updatemembercount($uid, array(
                        $this->setting['inviteconfig']['inviterewardcredit'] => $this->setting['inviteconfig']['inviteaddcredit']
                    ));
                }
                if ($this->setting['inviteconfig']['invitedaddcredit']) {
                    updatemembercount($invite['uid'], array(
                        $this->setting['inviteconfig']['inviterewardcredit'] => $this->setting['inviteconfig']['invitedaddcredit']
                    ));
                }
                require_once libfile('function/friend');
                friend_make($invite['uid'], $invite['username'], false);
                notification_add($invite['uid'], 'friend', 'invite_friend', array(
                    'actor' => '<a href="home.php?mod=space&uid=' . $invite['uid'] . '" target="_blank">' . $invite['username'] . '</a>'
                ), 1);
            ...
            if ($welcomemsg && ! empty($welcomemsgtxt)) {
                ...
                if ($welcomemsg == 1) {
                    ...
                    notification_add($uid, 'system', $welcomemsgtxt, array(
                        'from_id' => 0,
            ...
            $extra = array(
                'showid' => 'succeedmessage',
                'extrajs' => '<script type="text/javascript">' . 'setTimeout("window.location.href =\'' . $href . '\';", ' . $refreshtime . ');' . '$(\'succeedmessage_href\').href = \'' . $href . '\';' . '$(\'main_message\').style.display = \'none\';' . '$(\'main_succeed\').style.display = \'\';' . '$(\'succeedlocation\').innerHTML = \'' . lang('message', $locationmessage) . '\';' . '</script>',
                'striptags' => false
            );
    ...
    function on_registerOutsideLogin()
    {
        global $_G;
        $_GET['username'] = trim($_GET['phone']);
        $_GET['password'] = $_GET['' . $this->setting['reginput']['password']];
        // $_GET['password2'] = $_GET[''.$this->setting['reginput']['password2']];
        // $_GET['email'] = $_GET[''.$this->setting['reginput']['email']];
        $_GET['password2'] = $_GET['' . $this->setting['reginput']['password']];
        /* if (!$_POST['emil']){
            $_GET['email'] = $_GET['username']."@afmobi.group";
        }else {
            $_GET['email'] = getgpc('email');
        ...
        if (empty($_GET['username'])) {
            ...
            foreach ($_G['cache']['fields_register'] as $field) {
                ...
                if (! profile_check($field_key, $field_val) && empty($field_val)) {
                    $showid = ! in_array($field['fieldid'], array(
                        'birthyear',
                        'birthmonth'
                    )) ? $field['fieldid'] : 'birthday';
    ...
    function on_register()
    {
        ...
        if (! $sendurl) {
            if (! empty($_GET['hash'])) {
                $_GET['hash'] = preg_replace("/[^\[A-Za-z0-9_\]%\s+-\/=]/", '', $_GET['hash']);
                $hash = explode("\t", authcode($_GET['hash'], 'DECODE', $_G['config']['security']['authkey']));
                if (is_array($hash) && isemail($hash[0]) && TIMESTAMP - $hash[1] < 259200) { // 判断邮箱验证的串码，是否重复验证
                    $sendurl = false;
                }
            }
        }
        if (! submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
            ...
            if (! $sendurl) {
                ...
                $uid = C::t('common_member')->max_uid() + 1;
                $hashstr = urlencode(authcode("$_GET[email]\t$_G[timestamp]", 'ENCODE', $_G['config']['security']['authkey']));
                $registerurl = "{$_G[siteurl]}home.php?mod=space&amp;uid={$uid}&amp;hash={$hashstr}&amp;email={$_GET[email]}";
                $email_register_message = lang('email', 'email_register_message', array(
                    'bbname' => $this->setting['bbname'],
                    'siteurl' => $_G['siteurl'],
                    'url' => $registerurl
                ...
                // showmessage('register_email_send_succeed', dreferer(), array('bbname' => $this->setting['bbname']), array('showdialog' => false, 'msgtype' => 3, 'closetime' => 10), '', $state=1);
            }
            //$emailstatus = 0;
            if (defined('IN_MOBILE_API')){
                $emailstatus = 1;
            ...
            if (! $activation) { // 表单验证
                ...
                $usernamelen = dstrlen($username);
                /* vot debug */
                $username_mblen = dstrlen($username);
                /* vot */
                if ($username_mblen < 2) {
                    showmessage('profile_username_tooshort');
                    /* vot */
                } elseif ($username_mblen > 15) {
                    /* vot */
                    showmessage('profile_username_toolong');
                    /* vot */
            ...
            if ($this->setting['regfloodctrl']) {
                $regip = C::t('common_regip')->fetch_by_ip_dateline($_G['clientip'], $_G['timestamp'] - 86400);
                if ($regip && $_G['clientip'] != '54.77.140.112') {    /* 限制同一IP注册过多用户除54.77.140.112外   2015.10.20*/ 
                    if ($regip['count'] >= $this->setting['regfloodctrl']) {
                        showmessage('register_flood_ctrl', NULL, array(
                            'regfloodctrl' => $this->setting['regfloodctrl']
                        ));
