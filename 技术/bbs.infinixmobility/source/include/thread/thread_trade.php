...
if(empty($_GET['do']) || $_GET['do'] == 'tradeinfo') {
	...
	if($_GET['do'] == 'tradeinfo') {
        ...
        $trade['credits'] = getuserprofile('extcredits'.$_G['setting']['creditstransextra'][5]);  //陈焰辉 2015.12.22 修改积分兑换
        $trade['offset'] =  floor($trade['credits']/100);            //陈焰辉 2015.12.22 修改积分兑换
		...
		$comments = array();
		foreach(C::t('forum_tradecomment')->fetch_all_list_by_pid($_GET['pid']) as $comment) {
