...
if (submitcheck('profilesubmit')) {
    // if(!empty($_GET['bloodtype']) || !empty($_GET['formhash'])){
    ...
    foreach ($_POST as $key => $value) {
        ...
        if (in_array($field['formtype'], array(
            'text',
            'textarea'
        )) || in_array($key, $forumfield)) {
            $censor->check($value);
        ...
        if (in_array($key, $forumfield)) {
            ...
            if ($value >= - 12 && $value <= 12 || $value == 9999) {
                C::t('common_member')->update($_G['uid'], array(
                    'timeoffset' => intval($value)
                ));
            }
        } elseif ($key == 'site') {
            if (! in_array(strtolower(substr($value, 0, 6)), array(
                'http:/',
                'https:',
                'ftp://',
                'rtsp:/',
                'mms://'
            )) && ! preg_match('/^static\//', $value) && ! preg_match('/^data\//', $value)) {
                $value = 'http://' . $value;
            }
        }
    ...
    if (isset($_POST['privacy'])) {
        ...
        C::t('common_member_field_home')->update($space['uid'], array(
            'privacy' => serialize($space['privacy'])
        ));
    }
    ...
    $message = $vid ? lang('spacecp', 'profile_verify_verifying', array(
        'verify' => $verifyconfig['title']
    )) : '';
    if ($_GET['mobile'] == '2') {
...
//} elseif (submitcheck('passwordsubmit', 0, $seccodecheck, $secqaacheck)) {
    ...
    if (! empty($_GET['newpassword']) && $_GET['newpassword'] != addslashes($_GET['newpassword'])) {
        showmessage('profile_passwd_illegal', '', array(), array(
            'return' => true
        ));
    }
    if (! empty($_GET['newpassword']) && $_GET['newpassword'] != $_GET['newpassword2']) {
        showmessage('profile_passwd_notmatch', '', array(), array(
            'return' => true
        ));
    }
    ...
    if ($ucresult == - 1) {
        showmessage('profile_passwd_wrong', '', array(), array(
            'return' => true
        ));
    } elseif ($ucresult == - 4) {
        showmessage('profile_email_illegal', '', array(), array(
            'return' => true
        ));
    } elseif ($ucresult == - 5) {
        showmessage('profile_email_domain_illegal', '', array(), array(
            'return' => true
        ));
    } elseif ($ucresult == - 6) {
        showmessage('profile_email_duplicate', '', array(), array(
            'return' => true
        ));
    }
    ...
    if (in_array('mobile', $_G['setting']['plugins']['available']) && $wechatuser['isregister']) {
        C::t('#wechat#common_member_wechat')->update($_G['uid'], array(
            'isregister' => 0
        ));
    }
    ...
    if ($_G['member']['freeze'] == 2) {
        C::t('common_member_validate')->update($_G['uid'], array(
            'message' => dhtmlspecialchars($_G['gp_freezereson'])
        ));
    }
...
if ($operation == 'password') {
    ...
    if (! empty($space['newemail'])) {
        $acitvemessage = lang('spacecp', 'email_acitve_message', array(
...
if (! isset($without_tpl)) {
    $without_tpl = false;
}

if ($without_tpl == false) {
