...
if($_GET['op'] == 'delete') {
	...
	if($_GET['checkall']) {
		...
		if(submitcheck('deletesubmit')) {
			...
			showmessage('do_success', 'home.php?mod=space&uid='.$_G['uid'].'&do=favorite&view=me&type='.$_GET['type'].'&quickforward=1', array('favid' => $favid, 'id' => $thevalue['id']), array('showdialog'=>1, 'showmsg' => true, 'closetime' => false, 'locationtime' => 3));
		}
	}
	...
	if(submitcheck('favoritesubmit') || ($type == 'forum' || $type == 'group' || $type == 'thread') || (defined('IN_MOBILE_API') && IN_MOBILE_API)) {
		$arr = array(
			'uid' => intval($_G['uid']),
			'idtype' => $idtype,
			'id' => $id,
