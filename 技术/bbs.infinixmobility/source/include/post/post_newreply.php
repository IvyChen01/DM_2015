...
if($_G['setting']['commentnumber'] && !empty($_GET['comment'])) {
	if((!defined('IN_MOBILE_API') || !IN_MOBILE_API) && !submitcheck('commentsubmit', 0, $seccodecheck, $secqaacheck)) {
...
if((!defined('IN_MOBILE_API') || !IN_MOBILE_API) && !submitcheck('replysubmit', 0, $seccodecheck, $secqaacheck)) {
...
/*vot*/		//$post_reply_quote = '[img]static/image/common/user_online.gif[/img] '.$thaquote['author'].' [img]static/image/common/clock.gif[/img] '.$time;
	...
	if (defined('IN_MOBILE_API') && IN_MOBILE_API && !empty($_GET['repquote'])) {
		$thaquote = C::t('forum_post')->fetch('tid:'.$_G['tid'], $_GET['repquote']);
		
		if ($thaquote['first'] == 0) {
			$message = $thaquote['message'];
			$quote_pos = strpos($message, '[/quote]');
			if ($quote_pos) {
				$message=preg_replace("/\[quote\]((.|\n)*)\[\/quote\]/", '', $message);
			}
		    $time = dgmdate($thaquote['dateline']);
		    $post_reply_quote = lang('forum/misc', 'post_reply_quote', array('author' => $thaquote['author'], 'time' => $time));
		    $message = "[quote][size=2][url=forum.php?mod=redirect&goto=findpost&pid=$_GET[pid]&ptid={$_G['tid']}][color=#999999]{$post_reply_quote}[/color][/url][/size]\n{$message}[/quote]";
