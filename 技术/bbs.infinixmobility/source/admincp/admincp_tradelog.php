...
showsubmenu('nav_ec', array(
	...
	array('tradelog export', 'tradelog&mod=forum&do=export', 1),
));
showtableheader();
...
foreach(C::t('forum_tradelog')->fetch_all_by_status($filter, $start_limit, $ppp) as $tradelog) {
	...
	//$tradelog_export[] = $tradelog; /* 订单导出  chenyanhui 15.12.23*/
}
/* 订单导出  chenyanhui 15.12.23*/
if($_GET['do'] == 'export') {
    $tradelog_export = C::t('forum_tradelog')->fetch_all_by_status($filter, $start_limit, $ppp);
    foreach ($tradelog_export as $key=>$value){
        $tradelog_export[$key]['status'] = strip_tags($value['status']);
    }
    
    $filename = 'tradelog.xls';
    include template('forum/tradelog_export');
