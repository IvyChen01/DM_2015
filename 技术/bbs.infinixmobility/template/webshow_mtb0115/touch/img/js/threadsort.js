...
function changeselectthreadsort(selectchoiceoptionid, optionid, type) {
	...
	for(var i in choicesarr) {
		...
		if(choicesarr[i]['scount'] != lastcount) {
			...
			selectoption += '</select>' + "\r\n" + '<select' + nameid + ' class="ps vm" onchange="changeselectthreadsort(this.value, \'' + optionid + '\'' + issearch + ');checkoption(\'' + forum_optionlist[soptionid]['sidentifier'] + '\', \'' + forum_optionlist[soptionid]['srequired'] + '\', \'' + forum_optionlist[soptionid]['stype'] + '\')" ' + ((forum_optionlist[soptionid]['sunchangeable'] == 1 && type == 'update') ? 'disabled' : '') + '><option value="0">Select</option>';
	...
	getElem('select_' + forum_optionlist[soptionid]['sidentifier']).innerHTML = selectoption;
}

function checkoption(identifier, required, checktype, checkmaxnum, checkminnum, checkmaxlength) {
	if(checktype != 'image' && checktype != 'select' && !getElem('typeoption_' + identifier) || !getElem('check' + identifier)) {
		return true;
	}

	var ce = getElem('check' + identifier);
	ce.innerHTML = '';

	if(checktype == 'select') {
		if(required != '0' && (getElem('typeoption_' + identifier) == null || getElem('typeoption_' + identifier).value == '0')) {
			warning(ce, 'Necessary info is missing');
			return false;
		} else if(required == '0' && (getElem('typeoption_' + identifier) == null || getElem('typeoption_' + identifier).value == '0')) {
			ce.innerHTML = '<img src="' + IMGDIR + '/check_error.gif" width="16" height="16" class="vm" /> Please select next level';
	...
	if(checktype == 'radio' || checktype == 'checkbox') {
		var nodes = getElem('typeoption_' + identifier).parentNode.parentNode.parentNode.getElementsByTagName('INPUT');
	...
	if(checktype == 'image') {
		var checkvalue = getElem('sortaid_' + identifier).value;
	} else {
		var checkvalue = getElem('typeoption_' + identifier).value;
	}
