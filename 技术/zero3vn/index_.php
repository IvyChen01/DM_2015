...
class Game
{
	...
	public function addRestLucky($fbid)
	{
		Config::$db->connect();
		$sqlFbid = Security::varSql($fbid);
		$tbUser = Config::$tbUser;
	...
	public function checkShared($fbid)
	{
		Config::$db->connect();
		$tbUser = Config::$tbUser;
		$sqlFbid = Security::varSql($fbid);
		Config::$db->query("select isshared from $tbUser where fbid=$sqlFbid");
		$res = Config::$db->getRow();
		if (empty($res))
		{
			return false;
	...
	public function setShare($fbid)
	{
		Config::$db->connect();
		$tbUser = Config::$tbUser;
		$sqlFbid = Security::varSql($fbid);
		Config::$db->query("update $tbUser set isshared=1 where fbid=$sqlFbid");
	}
}
...
class System
{
	...
	/**
	 * 上传图片 (待修改，将json_encode转到控制器处理，增加返回file参数，参考zero2hero)
	 */
