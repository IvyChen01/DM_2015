...
class GameController
{
	...
	private function doAnswer()
	{
		...
		$q3 = trim(Security::varPost('q3'));
		$qs3 = trim(Security::varPost('qs3'));
		$q4 = (int)Security::varPost('q4');
		$q5 = (int)Security::varPost('q5');
		$q6 = (int)Security::varPost('q6');
		$q7 = (int)Security::varPost('q7');
		$q8 = trim(Security::varPost('q8'));
		$q9 = trim(Security::varPost('q9'));
		
		if (($q1 >= 1 && $q1 <=2) && ($q2 >= 1 && $q2 <=4) && ($q4 >= 1 && $q4 <=2) && ($q5 >= 1 && $q5 <=4) && ($q6 >= 1 && $q6 <=2) && ($q7 >= 1 && $q7 <=4))
		{
			$this->game->setAnswer($this->fbid, $q1, $q2, $q3, $qs3, $q4, $q5, $q6, $q7, $q8, $q9);
			System::echoData(Config::$msg['ok']);
		}
		else
		{
...
class Config
{
	public static $configType = 2;//配置方案。1：local，2：Tecno SSD
	...
	public static function init()
	{
		...
		switch (self::$configType)
		{
			case 1:
				...
				self::$systemName = 'surveyssd';//系统名称
				self::$key = 'f1a*sf_rve.y12ad@#&b';//密钥
			...
			case 2:
				...
				self::$systemName = 'surveyssd';//系统名称
				self::$key = 'y.13a&f^v1ey.()1!3';//密钥
				self::$baseUrl = 'http://www.tecno-mobile.com/surveyssd';//当前网址，线上或本地
				self::$resUrl = '';//资源文件地址
				self::$newsEnabled = false;//新闻开关
				self::$zhuanPanEnabled = false;//转盘开关
				self::$hongBaoEnabled = false;//红包开关
				...
				self::$fbAppId = '1134798909865422';//facebook app id
				self::$fbAppKey = 'fb7d5627426a01c7c5f6bfb9a8d0b3fe';//facebook app key
				self::$fbAppRedirectUrl = 'https://www.tecno-mobile.com/surveyssd/';//facebook app跳转地址
				self::$fbAppUrl = 'https://apps.facebook.com/surveyssd/';//facebook app地址
				self::$siteUrl = 'http://www.tecno-mobile.com/surveyssd/?a=main';
				self::$shareUrl = 'http://www.tecno-mobile.com/surveyssd/?shareId=';
				self::$sharePic = 'http://www.tecno-mobile.com/surveyssd/images/share.jpg';
				self::$dbConfig = self::$dbSsd;//数据库配置信息，线上或本地
				self::$countCode = '';//统计代码
				self::$msg = array
				(
					//通用
...
class Game
{
	...
	public function exportUser()
	{
		...
		*/
		...
		$excel->getActiveSheet()->getStyle('O')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		...
		*/
		...
		foreach ($res as $row)
		{
			$excel->getActiveSheet()
				...
				->setCellValue('G' . $rowIndex, $row['q2'])
				->setCellValue('H' . $rowIndex, $row['q3'])
				->setCellValue('I' . $rowIndex, $row['qs3'])
				->setCellValue('J' . $rowIndex, $row['q4'])
				->setCellValue('K' . $rowIndex, $row['q5'])
				->setCellValue('L' . $rowIndex, $row['q6'])
				->setCellValue('M' . $rowIndex, $row['q7'])
				->setCellValue('N' . $rowIndex, $row['q8'])
				->setCellValue('O' . $rowIndex, $row['q9']);
		...
		{
			//输出EXCEL格式
			$filename = 'ssd_survey_' . Utils::mdate('Y.m.d') . '.xls';
		...
		{
			//输出PDF格式
			$filename = 'ssd_survey_' . Utils::mdate('Y.m.d') . '.pdf';
	...
	public function setAnswer($fbid, $q1, $q2, $q3, $qs3, $q4, $q5, $q6, $q7, $q8, $q9)
	{
		Config::$db->connect();
		$sqlFbid = Security::varSql($fbid);
		$sqlQ1 = (int)$q1;
		...
		$sqlQs3 = Security::varSql($qs3);
		$sqlQ4 = (int)$q4;
		$sqlQ5 = (int)$q5;
		$sqlQ6 = (int)$q6;
		$sqlQ7 = (int)$q7;
		$sqlQ8 = Security::varSql($q8);
		$sqlQ9 = Security::varSql($q9);
		$sqlDate = Security::varSql(Utils::mdate('Y-m-d H:i:s'));
		$tbAnswer = Config::$tbAnswer;
		$tbUser = Config::$tbUser;
		Config::$db->query("insert into $tbAnswer (fbid, q1, q2, q3, qs3, q4, q5, q6, q7, q8, q9) values ($sqlFbid, $sqlQ1, $sqlQ2, $sqlQ3, $sqlQs3, $sqlQ4, $sqlQ5, $sqlQ6, $sqlQ7, $sqlQ8, $sqlQ9)");
