...
class Game
{
		...
		*/
		...
		foreach ($res as $row)
		{
			$excel->getActiveSheet()
				...
				->setCellValue('E' . $rowIndex, $row['answer'])
				->setCellValue('F' . $rowIndex, $row['q1'])
				->setCellValue('G' . $rowIndex, $row['q2'])
				->setCellValue('H' . $rowIndex, $row['q3'])
				->setCellValue('I' . $rowIndex, $row['q4'])
				->setCellValue('J' . $rowIndex, $row['q5'])
				->setCellValue('K' . $rowIndex, $row['q6'])
				->setCellValue('L' . $rowIndex, $row['q7'])
				->setCellValue('M' . $rowIndex, $row['q8'])
				->setCellValue('N' . $rowIndex, $row['q9'])
				->setCellValue('O' . $rowIndex, $row['q10']);
		...
		{
			//输出EXCEL格式
			$filename = 'about_infinix_' . Utils::mdate('Y.m.d') . '.xls';
		...
		{
			//输出PDF格式
			$filename = 'about_infinix_' . Utils::mdate('Y.m.d') . '.pdf';
	...
	public function setAnswer($fbid, $q1, $q2, $q3, $q4, $q5, $q6, $q7, $q8, $q9, $q10)
	{
		Config::$db->connect();
		$sqlFbid = Security::varSql($fbid);
		$sqlQ1 = (int)$q1;
		...
		$tbUser = Config::$tbUser;
		Config::$db->query("insert into $tbAnswer (fbid, q1, q2, q3, q4, q5, q6, q7, q8, q9, q10) values($sqlFbid, $sqlQ1, $sqlQ2, $sqlQ3, $sqlQ4, $sqlQ5, $sqlQ6, $sqlQ7, $sqlQ8, $sqlQ9, $sqlQ10)");
	...
	public function getAnswerData()
	{
		...
		{
			if ($value['q1'] == 1 && $value['q2'] == 2 && $value['q3'] == 2 && $value['q4'] == 2 && $value['q5'] == 3 && $value['q6'] == 2 && $value['q7'] == 2 && $value['q8'] == 3 && $value['q9'] == 3 && $value['q10'] == 3)
			{
				$res[$key]['answer'] = 'right';
			}
			else
			{
				$res[$key]['answer'] = 'wrong';
			}
		}
		return $res;
	}
