<?php if (!defined('THINK_PATH')) exit();?><div class="bjui-pageHeader">
	<form action="<?php echo U(edit);?>"  method="POST" class="pageForm" data-toggle="validate" data-reload="true">
		...
		<input placeholder='<?php echo (L("goods_cats_name")); ?>' id="newName" type="text" name="catName" value="" />
		<input placeholder='<?php echo (L("sort")); ?>' id="catId" type="text" name="catSort" value=""  />
	...
	<table class="table table-bordered table-hover table-striped table-top" data-layout-h="0" data-selected-multi="true">
		<thead>
			<tr>
				<!-- <th>序号</th> -->
				<th>ID</th>
				<th><?php echo (L("cats_name")); ?></th>
				<th><?php echo (L("cat_pic")); ?></th>
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tree): $mod = ($i % 2 );++$i;?><tr id='tr_0_<?php echo ($i); ?>' class="tr_0">
				...
				<td>
				<a href="<?php echo U('del?id=' . $tree[catId]);?>" class="btn btn-red glyphicon glyphicon-trash" data-toggle="doajax" data-confirm-msg="确定要删除该行信息吗？"><?php echo (L("delete")); ?></a>
				<a href="<?php echo U('toEdit?id=' . $tree[catId]);?>" class="btn btn-default glyphicon glyphicon-pencil" data-toggle="dialog" data-width="800" data-height="600" data-id="dialog-mask" data-mask="true"><?php echo (L("edit")); ?></a>
				<a class="btn btn-default glyphicon glyphicon-plus" data-toggle="dialog" data-width="800" data-height="600" data-id="dialog-mask" data-mask="true" href="<?php echo U('toEdit',array('parentId'=>$tree[catId]));?>" ><?php echo (L("add_child")); ?></a>&nbsp;
