	 ...
	 */
	  ...
	  */
	  public function queryByList(){
	     $m = M('goods');
	     $sql = "select * from __PREFIX__goods where goodsStatus=1 and goodsFlag=1 order by goodsId desc";
		 return $m->query($sql);
	  }
	  /**
	   * 获取列表
	   */
	  public function queryByListlang($lang){
	      $m = M('goods');
	      $sql = "select * from __PREFIX__goods where goodsStatus=1 and goodsFlag=1 and lang='$lang' order by goodsId desc";
	      return $m->query($sql);
	  }
	  /**
	   * 获取同一分类的商品
	  ...
	  public function getGoodsByPid($id){
	     $m = M('goods');
	     $rs = $m->where(array('goodsStatus'=>1,'goodsFlag'=>1,'goodsCatId1'=>$id))->select(); 
		 return $rs;
	  }
	  /**
	   * 获取同一分类的商品
	   */
	  public function getGoodsByPidlang($id,$lang){
	      $m = M('goods');
	      $rs = $m->where(array('goodsStatus'=>1,'goodsFlag'=>1,'goodsCatId1'=>$id,'lang'=>$lang))->select();
	      return $rs;
	  }
	 /**
	  * 修改商品状态
	  ...
	  * 获取推荐商品
	  */
	 public function getRecommGoodslang($lang){
	     $m = M('goods');
	     $rs = $m->where(array('goodsStatus'=>1,'goodsFlag'=>1,'isRecomm'=>1,'lang'=>$lang))->select();
	     return $rs;
	 }
	 /* 
	  * 根据id获取商品详情信息
	 ...
	 public function getGoodsImgsById($id){
	    $m = D('goods_gallerys');
	    $rs = $m->where(array('goodsId'=>$id))->select();
	    return $rs;
	 }
	 /**
	  +----------------------------------------------------------
