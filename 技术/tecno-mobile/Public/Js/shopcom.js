...
function queryUnSaleByPage(){
	...
	location.href= Think.U('Admin/Goods/queryUnSaleByPage','goodsName='+goodsName+"&shopCatId1="+shopCatId1+"&shopCatId2="+shopCatId2); 
}
function queryOnSale(){
	var shopCatId1 = $('#shopCatId1').val();
	var shopCatId2 = $('#shopCatId2').val();
	var goodsName = $('#goodsName').val();
	location.href= Think.U('Admin/Goods/queryOnSaleByPage','goodsName='+goodsName+"&shopCatId1="+shopCatId1+"&shopCatId2="+shopCatId2); 
}
function queryPendding(){
	var shopCatId1 = $('#shopCatId1').val();
	var shopCatId2 = $('#shopCatId2').val();
	var goodsName = $('#goodsName').val();
	location.href= Think.U('Admin/Goods/queryPenddingByPage','goodsName='+goodsName+"&shopCatId1="+shopCatId1+"&shopCatId2="+shopCatId2); 
}
function toEditGoods(id,menuId){
	location.href= Think.U('Admin/Goods/toEdit','umark='+menuId+"&id="+id); 
}
function toViewGoods(id){
	$.post(Think.U('Admin/Goods/getGoodsVerify'),{id:id},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
			var verifyCode = json.verifyCode;
			window.open(Think.U('Admin/Goods/getGoodsDetails','goodsId='+id+"&kcode="+verifyCode));
		}
	});
...
function delGoods(id){
		...
		$.post(Think.U('Admin/Goods/del'),{id:id},function(data,textStatus){
			layer.close(tips);
    		var json = WST.toJson(data);
    		if(json.status=='1'){
...
function batchDel(){
	layer.confirm("您确定要删改这些商品？",{icon: 3, title:'系统提示'},function(){
	      ...
	      var params = {};
	      params.ids = ids;
	      $.post(Think.U('Admin/Goods/batchDel'),params,function(data,textStatus){
	    		var json = WST.toJson(data);
	    		if(json.status=='1'){
...
function sale(v){
	...
	layer.confirm((v==1)?"您确定要上架这些商品？":"您确定要下架这些商品？",{icon: 3, title:'系统提示'},function(tips){
	    ...
	    params.ids = ids;
	    params.isSale= v;
	    $.post(Think.U('Admin/Goods/sale'),params,function(data,textStatus){
	    	layer.close(loading);
	    	var json = WST.toJson(data);
	    	if(json.status=='1'){
	    		WST.msg('操作成功！', {icon: 1},function(){
...
function goodsSet(type,umark){
	...
	params.ids = ids;
	params.code= type;
	$.post(Think.U('Admin/Goods/goodsSet'),params,function(data,textStatus){
	    var json = WST.toJson(data);
	    if(json.status=='1'){
	    	WST.msg('操作成功！', {icon: 1},function(){
	    		location.reload();
...
function getShopCatListForGoods(v,id){
	   ...
	   $('#shopCatId2').empty();
	   var html = [];
	   $.post(Think.U('Admin/ShopsCats/queryByList'),params,function(data,textStatus){
...
function getBrands(catId){
	...
	$.post(Think.U('Admin/Brands/queryBrandsByCat'),params,function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1' && json.list){
			for(var i=0;i<json.list.length;i++){
				opts = json.list[i];
...
function getCatListForEdit(objId,parentId,t,id){
	   ...
	   if(t<1){
		   ...
		   getBrands(parentId);
		   if(!id){
			   getAttrList(parentId);
		   }
	   }
	   var html = [];
	   $.post(Think.U('Admin/GoodsCats/queryByList'),params,function(data,textStatus){
		    html.push('<option value="">请选择</option>');
			var json = WST.toJson(data);
			if(json.status=='1' && json.list){
				var opts = null;
...
function editGoods(){
	   ...
	   params.goodsImg = $('#j_custom_pic').val();
	   ...
	   $.post(Think.U('Admin/Goods/edit'),params,function(data,textStatus){
		   layer.close(loading);
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('操作成功!', {icon: 1}, function(){
				...
				if(params.isSale==1){
				    WST.msg('您的店铺已被封，如有疑问请与商城管理员联系!', {icon: 5,time:4000},function(){
				    	if((menuId=='toEditGoods')){
							location.href= Think.U('Admin/Goods/toEdit','umark=toEditGoods');
						}else{
							location.href=Think.U('Admin/Goods/'+menuId);
						}
				    });
				}else{
					WST.msg('操作成功!', {icon: 1}, function(){
...
function getAttrList(catId){
	...
	$.post(Think.U('Admin/Attributes/getAttributes'),{catId:catId},function(data,textStatus){
		 var json = WST.toJson(data);
		 var priceAttr = null;
		 if(json.status=='1' && json.list){
			$("#goodsAttr").show();
...
function addPriceAttr(){
	...
	html.push('<td><input type="text" id="price_name_'+obj.attrId+'_'+goodsPriceNo+'" name="price_name_'+obj.attrId+'_'+goodsPriceNo+'"/></td>');
	html.push('<td><input type="text" id="price_price_'+obj.attrId+'_'+goodsPriceNo+'" name="price_price_'+obj.attrId+'_'+goodsPriceNo+'" value="0" onblur="checkAttPrice('+obj.attrId+','+goodsPriceNo+');" onkeypress="return WST.isNumberdoteKey(event)" onkeyup="javascript:WST.isChinese(this,1)" maxLength="10"/></td>');
	...
	if(goodsPriceNo==1){
		html.push('<td><a title="新增" class="add btn" href="javascript:addPriceAttr()">新增</a></td>');
	}else{
	    html.push('<td><a title="删除" class="del btn" href="javascript:delPriceAttr('+goodsPriceNo+')">删除</a></td></tr>');
	}
...
function addAttr(obj){
	...
	if(obj.attrType==0){
		...
		if(obj.opts && obj.opts.txt){
			html.push("<input type='hidden' dataType='"+obj.attrType+"' dataId='"+obj.attrId+"' name='attr_name_"+obj.attrId+"' id='attr_name_"+obj.attrId+"' class='attrList'>");
		...
		html.push("<select class='attrList' id='attr_name_"+obj.attrId+"' name='attr_name_"+obj.attrId+"' dataId='"+obj.attrId+"'>");
        if(obj.opts && obj.opts.txt){
			for(var i=0;i<obj.opts.txt.length;i++){
...
function editGoodsCat(){
	   ...
	   $.post(Think.U('Admin/ShopsCats/edit'),params,function(data,textStatus){
		   layer.close(loading);
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('操作成功!', {icon: 1}, function(){
				   location.href= Think.U('Admin/ShopsCats/index');
				});
			}else{
...
function delGoodsCat(id){
	layer.confirm("您确定要删除该商品分类吗？",{icon: 3, title:'系统提示'},function(tips){
		layer.load('正在处理，请稍后...', 3);
		layer.close(tips);
		$.post(Think.U('Admin/ShopsCats/del'),{id:id},function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('操作成功!', {icon: 1}, function(){
					location.reload();
...
function editGoodsCatName(obj){
	var name = $('#');
	$.post(Think.U('Admin/ShopsCats/editName'),{id:$(obj).attr('dataId'),catName:obj.value},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
			WST.msg('操作成功!',{icon: 1});
		}else{
...
function queryAppraises(){
	...
	var shopCatId2 = $('#shopCatId2').val();
	var goodsName = $('#goodsName').val();
	location.href= Think.U('Admin/GoodsAppraises/index','umark=GoodsAppraises&goodsName='+goodsName+"&shopCatId1="+shopCatId1+"&shopCatId2="+shopCatId2);
}
...
function showOrder(id){
	layer.open({
	    ...
	    shade: [0.6, '#000'],
	    border: [0],
	    content: [Think.U('Admin/Orders/getOrderDetails','orderId='+id)],
...
function shopOrderAccept(id){
	layer.confirm('您确定受理该订单吗？', {icon: 3, title:'系统提示'}, function(tips){
	    ...
	    $.post(Think.U('Admin/Orders/shopOrderAccept'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function shopOrderProduce(id){
	layer.confirm('确定打包中吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Admin/Orders/shopOrderProduce'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function shopOrderDelivery(id){
	layer.confirm('确定正在发货吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Admin/Orders/shopOrderDelivery'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function shopOrderReceipt(id){
	layer.confirm('确定已收货吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Admin/Orders/shopOrderReceipt'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function shopOrderRefund(id){
	layer.confirm('确定拒收吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Admin/Orders/shopOrderRefund'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function shopOrderCancel(id){
	layer.confirm('确定取消吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Admin/Orders/shopOrderCancel'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function orderPager(statusMark,pcurr){
	...
	$.post(Think.U('Admin/Orders/queryShopOrders'),{orderNo:orderNo,userName:userName,userAddress:userAddress,statusMark:statusMark,pcurr:pcurr},function(data,textStatus){
		var json = WST.toJson(data);
		var html = new Array();
...
function editPass(){
	   ...
	   $.post(Think.U('Admin/Users/editPass'),params,function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
...
function getCommunitysForShopEdit(){
	  ...
	  $.post(Think.U('Admin/Areas/getAreaAndCommunitysByList'),{areaId:areaId},function(data,textStatus){
			var json = data;
			if(json.list){
				var html = [];
				json = json.list;
...
function editShop(){
	   ...
	   $.post(Think.U('Admin/Shops/edit'),params,function(data,textStatus){
			var json = WST.toJson(data);
...
function setShop(){
	   ...
	   $.post(Think.U('Admin/Shops/editShopCfg'),params,function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('操作成功!', {icon: 1}, function(){
					location.href=location.href;
...
function logout(){
	jQuery.post(Think.U('Admin/Shops/logout'),{},function(rsp) {
		location.reload();
	});
}
function checkLogin(){
	jQuery.post(Think.U('Admin/Shops/checkLoginStatus'),{},function(rsp) {
...
function editAttrCats(type,src){
	...
	var loading = layer.load('正在处理，请稍后...', 3);
	$.post(Think.U('Admin/AttributeCats/edit'),{umark:src,catName:catName,id:$('#id').val()},function(data,textStatus){
		layer.close(loading);
		var json = WST.toJson(data);
		if(json.status=='1'){
			WST.msg('操作成功!', {icon: 1}, function(){
				if(type==1){
				    ...
				    location.href=Think.U('Admin/AttributeCats/index');
				}
			});
		}else{
			WST.msg('操作失败!', {icon: 5});
...
function delAttrCat(id){
	layer.confirm("您确定要删除该商品类型及其下的属性吗？",{icon: 3, title:'系统提示'},function(tips){
	    ...
	    layer.close(tips);
	    var params = {};
	    $.post(Think.U('Admin/AttributeCats/del'),{id:id},function(data,textStatus){
	    	layer.close(loading);
	    	var json = WST.toJson(data);
	    	if(json.status=='1'){
	    		WST.msg('操作成功！', {icon: 1},function(){
...
function getAttrsForAttr(){
	location.href=Think.U("Admin/Attributes/index",'catId='+$('#catId').val());
}
function toAddAttr(){
...
function editAttrs(){
	...
	var loading = layer.load('正在处理，请稍后...', 3);
	$.post(Think.U('Admin/attributes/edit'),params,function(data,textStatus){
		layer.close(loading);
		var json = WST.toJson(data);
		if(json.status=='1'){
			WST.msg('操作成功!', {icon: 1}, function(){
...
function delAttrs(no,id){
	if(id>0){
		layer.confirm("您确定要删除该商品属性吗？",{icon: 3, title:'系统提示'},function(tips){
		    ...
		    layer.close(tips);
		    var params = {};
		    $.post(Think.U('Admin/Attributes/del'),{id:id},function(data,textStatus){
		    	layer.close(loading);
		    	var json = WST.toJson(data);
		    	if(json.status=='1'){
		    		WST.msg('操作成功！', {icon: 1},function(){
...
function batchMessageDel(){
	  layer.confirm("您确定要删除这些消息？",function(){
	        ...
	        var params = {};
	        params.ids = ids;
	        $.post(Think.U('Admin/Messages/batchDel'),params,function(data,textStatus){
	          var json = WST.toJson(data);
	          if(json.status=='1'){
	        	  WST.msg('操作成功！', {icon: 1},function(){
	              location.reload();
...
function editStock(goodsId){
	...
	jQuery.post(Think.U('Admin/Goods/editStock'),{goodsId:goodsId,stock:stock},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status>0){
...
function ChangeCatStatus(isShow,id,pid){
	...
	params.pid = pid;
	$.post(Think.U('Admin/ShopsCats/ChangeCatStatus'),params,function(data,textStatus){
		location.reload();  
	});
...
function changSaleStatus(goodsId,flag){
	...
	jQuery.post(Think.U('Admin/Goods/changSaleStatus'),{goodsId:goodsId,tamk:tamk,flag:flag},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status>0){
			if(tamk == 0){
				tamk = 1;
